<x-app-layout :title="__('Email log')" :header="__('Email log')">
    @php
        $typeMeta = [
            'quote'              => ['label' => __('Quote'),              'icon' => 'ri-file-list-3-line',  'color' => 'bg-blue-50 text-blue-700'],
            'order_confirmation' => ['label' => __('Order confirmation'), 'icon' => 'ri-file-paper-2-line', 'color' => 'bg-emerald-50 text-emerald-700'],
            'follow_up'          => ['label' => __('Follow-up'),          'icon' => 'ri-mail-send-line',    'color' => 'bg-amber-50 text-amber-700'],
        ];
    @endphp
    ...
    <form method="GET" action="{{ route('email-log.index') }}" class="mb-4 flex flex-wrap items-center gap-2">
        <div class="relative flex-1 min-w-[240px]">
            ...
            <input type="text" name="q" value="{{ $q }}"
                placeholder="{{ __('Search recipient, subject, quote ref…') }}"
        ...
        <select name="type" onchange="this.form.submit()"
            ...
            <option value="quote"              @selected($type === 'quote')>{{ __('Quote') }}</option>
            <option value="order_confirmation" @selected($type === 'order_confirmation')>{{ __('Order confirmation') }}</option>
            <option value="follow_up"          @selected($type === 'follow_up')>{{ __('Follow-up') }}</option>
        </select>
        ...
        @if ($q || $type || $status)
            <a href="{{ route('email-log.index') }}" class="text-xs text-gray-500 hover:text-gray-900 px-2">{{ __('Clear') }}</a>
        @endif
    </form>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase tracking-wide">{{ __('Total') }}</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $counts['all'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-emerald-700 uppercase tracking-wide font-medium">{{ __('Sent') }}</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $counts['sent'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-red-700 uppercase tracking-wide font-medium">{{ __('Failed') }}</p>
            <p class="text-2xl font-bold text-gray-900 mt-1">{{ $counts['failed'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500 uppercase tracking-wide">{{ __('Follow-ups') }}</p>
    ...
    @if ($logs->isEmpty())
        <x-app.empty-state
            ...
            :message="__('Outbound emails will appear here once you send your first quote.')"
            size="lg" />
    @else
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase text-gray-500 tracking-wide">
                    <tr>
                        ...
                        <th class="px-5 py-3 font-semibold">{{ __('Recipient') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('Subject') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('Quote') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('Sent by') }}</th>
                        <th class="px-5 py-3 font-semibold">{{ __('Sent') }}</th>
                            ...
                            <td class="px-5 py-3">
                                @if ($log->status === 'sent')
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-xs font-semibold">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>{{ __('Sent') }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-50 text-red-700 text-xs font-semibold" title="{{ $log->error_message }}">
                                        <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>{{ __('Failed') }}
                                    </span>
                                @endif
                            </td>

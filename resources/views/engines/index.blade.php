<x-app-layout title="Engines" header="Engines">
    @if (session('status'))
    ...
    <div class="mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        <form method="GET" action="{{ route('engines.index') }}" class="flex-1 max-w-md">
            <div class="relative">
                ...
                <input type="text" name="q" value="{{ $q }}"
                    placeholder="Search by brand, code, or description…"
        ...
        <a href="{{ route('engines.create') }}"
            ...
            <i class="ri-add-line"></i> Add engine
        </a>
    </div>
    ...
    @if ($engines->isEmpty())
        <x-app.empty-state
            ...
            message="Add the engine SKUs you carry (Suzuki DF200A, Yamaha F300, etc.). They'll be available to attach to any quote."
            ctaLabel="Add the first engine"
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase text-gray-500 tracking-wide">
                    <tr>
                        ...
                        <th class="px-5 py-3 font-semibold">HP</th>
                        <th class="px-5 py-3 font-semibold">Fuel</th>
                        <th class="px-5 py-3 font-semibold text-right">Public HT</th>
                        <th class="px-5 py-3 font-semibold text-right">VAT</th>
                        <th class="px-5 py-3 font-semibold text-right">TTC</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($engines as $engine)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-5 py-3 text-gray-700">{{ $engine->horsepower ? number_format($engine->horsepower, 0) . ' HP' : '—' }}</td>
                            ...
                            <td class="px-5 py-3">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <form method="POST" action="{{ route('engines.destroy', $engine->_id) }}"
                                        onsubmit="return confirm('Delete this engine?');" class="inline">
                                        @csrf @method('DELETE')
                                        <button class="inline-flex items-center justify-center w-8 h-8 text-red-600 hover:bg-red-50 rounded-lg" title="Delete">

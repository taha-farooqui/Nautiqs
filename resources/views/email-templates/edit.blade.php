<x-app-layout :title="__($meta['name']) . ' ' . __('email template')" :header="__($meta['name']) . ' ' . __('email template')">
    ...
    <div class="mb-4 flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('email-templates.index') }}" class="hover:text-primary-800">
            <i class="ri-arrow-left-line"></i> {{ __('All templates') }}
        </a>
    </div>
    ...
    <div class="mb-6 flex items-start gap-3 max-w-3xl">
        ...
        <div>
            <h2 class="font-semibold text-gray-900">{{ __($meta['name']) }}</h2>
            <p class="text-sm text-gray-600">{{ __($meta['description']) }}</p>
            <p class="text-xs text-gray-500 mt-1">
                {{ __('Variables like') }} <code class="font-mono px-1 py-0.5 bg-gray-100 rounded">@{{quote_number}}</code>
                {{ __('resolve at send time. Your company logo is added automatically at the top.') }}
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6"
        ...
        <form method="POST" action="{{ route('email-templates.update', $type) }}"
            ...
            {{-- Body --}}
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('Body') }}</label>
                ...
                <p class="text-xs text-gray-500 mt-2">
                    {{ __('Use the variable chips on the right to insert dynamic data like the client name or quote number.') }}
                </p>
            </div>
        ...
        <div class="xl:col-span-1 space-y-4">
            {{-- Variable picker --}}
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                <h3 class="font-semibold text-gray-900 text-sm mb-1">{{ __('Variables') }}</h3>
                <p class="text-xs text-gray-500 mb-3">{{ __('Click to insert into the body. Edit the Subject field directly to add them there.') }}</p>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden sticky top-20">
                <div class="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                    <h3 class="font-semibold text-gray-900 text-sm">{{ __('Live preview') }}</h3>
                    <span class="text-xs text-gray-500">{{ __('with sample data') }}</span>
                </div>
                <div class="p-4">
                    <div class="text-xs text-gray-400 mb-1">{{ __('SUBJECT') }}</div>
                    <div class="text-sm font-semibold text-gray-900 mb-4 break-words" x-text="renderedSubject()"></div>
                    <div class="text-xs text-gray-400 mb-1">{{ __('BODY') }}</div>
                    ...
                    <div class="border-t border-gray-100 pt-3">
                        ...
                        <div class="mt-6 pt-3 border-t border-gray-200 text-xs text-gray-500">
                            {{ __('Sent from') }} {{ $previewCompanyName }}
                        </div>
                    </div>
                </div>
            </div>
